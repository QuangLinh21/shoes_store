                    ...
                    <table class="table lms_table_active table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$item->cate_des}}</td>
                                <td>{{$item->cate_position}}</td>
                                <td>
                                    <?php
                                    if ($item->cate_status == 0) {
                                    ?>
                                    <a href="{{ URL::to('active-cate/' . $item->cate_id) }}"
                                        class="btn status_btn_st text-white">Ẩn</a>
                                    <?php
                                    } else {
                                    ?>
                                    <a href="{{ URL::to('unactive-cate/' . $item->cate_id) }}"
                                ...
                                <td>
                                    <div class="action_btns d-flex">
                                        <a href="{{route('category.edit',$item->cate_id)}}" class="action_btn mr_10"> <i
                                                class="far fa-edit"></i> </a>
                                        <a href="{{URL::to('delete_cate',$item->cate_id)}}" class="action_btn"  onclick="return confirm('Bạn có muốn xóa danh mục này không?')"> <i class="fas fa-trash"></i>
                                        </a>
                                    </div>
                                </td>
                            </tr>
